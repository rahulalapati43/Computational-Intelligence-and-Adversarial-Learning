...
def initializePopulation():
    ...
    for i in range(0,population_size):
        individual = []
        for j in range(0,95):
            individual.append(random.sample(random_list,1)[0])
        population[i] = individual

    return population

def evaluateFitness(raw_feature_file,CU_X,Y,child,num):
    transformed_file_name = raw_feature_file.split('.')[0] + "_EEDA_transformed_SEC_" + str(num) +".txt"
...
def selectParents(fitness):
    parents = []
    sorted_x = sorted(fitness.items(), key=operator.itemgetter(1), reverse=True)
...
def mutation(child):
    ...
    for i in range(0,len(child)):
        ...
        if random_number <= mutation_rate:
            if (child[i] == 1):
                mutate_child.append(0)
            elif (child[i] == 0):
                mutate_child.append(1)
        else:
...
def main(raw_feature_file):
    ...
    while (child_num < int(4975/24)):
        ...
        for i in range(0,24):
            ...
            child_fitness = evaluateFitness(raw_feature_file, CU_X, Y, mutate_child, i)
            print child_fitness
...
if __name__ == '__main__':
    ...
    output_file_name = "C:\Users\Buji\PycharmProjects\CI_Assignment_3\SEC_SportsWriters_Features\Elitist_EDA_Baseline_SEC_output.txt"
    ...
    for run in range(1, 11):
        ...
        plt.ylabel("Accuracy")
        plot_name = "C:\Users\Buji\PycharmProjects\CI_Assignment_3\Plots" + "\EEDA_Baseline_SEC_" + str(run) + ".png"
