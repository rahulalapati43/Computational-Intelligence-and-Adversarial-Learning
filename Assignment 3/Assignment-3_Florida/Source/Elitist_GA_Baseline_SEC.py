...
def evaluateFitness(raw_feature_file,CU_X,Y,child,num):
    transformed_file_name = raw_feature_file.split('.')[0] + "_EGA_transformed_SEC_" + str(num) +".txt"
...
def mutation(child_1,child_2):
    ...
    for i in range(0,len(child_1)):
        ...
        if random_number <= mutation_rate:
            if (child_1[i] == 1):
                mutate_child_1.append(0)
            elif (child_1[i] == 0):
                mutate_child_1.append(1)
        else:
    ...
    for i in range(0,len(child_2)):
        ...
        if random_number <= mutation_rate:
            if (child_2[i] == 1):
                mutate_child_2.append(0)
            elif (child_2[i] == 0):
                mutate_child_2.append(1)
        else:
...
if __name__ == '__main__':
    ...
    output_file_name = "C:\Users\Buji\PycharmProjects\CI_Assignment_3\SEC_SportsWriters_Features\Elitist_GA_Baseline_SEC_output.txt"
    ...
    for run in range(1, 11):
        ...
        plt.ylabel("Accuracy")
        plot_name = "C:\Users\Buji\PycharmProjects\CI_Assignment_3\Plots" + "\EGA_Baseline_SEC_" + str(run) + ".png"
