...
def main():
    ...
    parser.add_option('--sigma', dest='sigma',
                      default='',  # default empty!
                      help='sigma value')
    parser.add_option('--desired_outputs_vector', dest='desired_outputs_vector',
                      default='',  # default empty!
                      help='desired outputs vector')
    (options, args) = parser.parse_args()

    # assigning the user input
    feature_files = options.feature_files
    sigma = float(options.sigma)
    desired_outputs_vector = ast.literal_eval(options.desired_outputs_vector)
