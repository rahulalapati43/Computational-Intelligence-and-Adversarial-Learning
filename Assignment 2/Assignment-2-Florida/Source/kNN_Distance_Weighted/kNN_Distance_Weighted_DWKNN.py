...
def getLabel(neighbors,k):
    ...
    distance_k = float(neighbors[k-1][1])
    distance_1 = float(neighbors[0][1])
    labels = dict.fromkeys(keys, 0)
    for neighbor in neighbors:
        distance = float(neighbor[1])
        if (distance_k == distance_1):
            weight = 1
        else:
            weight = (((distance_k - distance)/(distance_k - distance_1)) * ((distance_k + distance)/(distance_k + distance_1)))
...
def main():
    ...
    for i in range(0,len(features)):
        ...
        # get predictions
        prediction = getLabel(neighbors,k)
